...
def get_711town_list(city: str, city_id: str):
    ...
    res = requests.get(
        SEVEN_ELEVEN_URL,
        params=params,
        headers={"Referer": "https://emap.pcsc.com.tw/"},
    )
...
def fetch_city_711(city: str, city_id: str):
    ...
    for town in towns:
        ...
        stores = get_711store_list(town["city"], town["town"])
...
def upload_711stores_to_firestore(stores):
    ...
    for store in stores:
        ...
        """doc_ref = (
            db.collection("7-11Store")
            .document(store["city"])
            .collection(store["area"])
            .document(store["id"])
        )"""
        doc_ref = db.collection("7-11Store").document(store["id"])
